/**
 * Euphoria.id — Investment Readiness Gate
...
const STORAGE_KEY = 'euphoria_history';
const STORAGE_LAST = 'euphoria_last_result';
...
const riskQuestions = [
  ...
  {
    id: 'r2',
    question: 'Horizon waktu investasi yang paling nyaman untukmu saat ini?',
    options: [
  ...
  {
    id: 'r3',
    question: 'Seberapa penting keamanan modal dibanding potensi return tinggi?',
    options: [
      { value: 1, text: 'Sangat penting — modal tidak boleh berkurang sama sekali' },
      { value: 2, text: 'Penting — return sedang sudah cukup asal stabil' },
      { value: 3, text: 'Seimbang — siap fluktuasi untuk return lebih baik' },
      { value: 4, text: 'Return tinggi lebih prioritas, fluktuasi bisa ditoleransi' }
    ]
  },
  {
    id: 'r4',
...
const literacyQuestions = [
  ...
  },
  {
    ...
    options: [
      { value: 0, text: 'Tetap investasi dulu karena return bisa lebih tinggi' },
  ...
  {
    id: 'l4',
    question: 'Return tinggi selalu berarti…',
    options: [
...
function nextStep(from) {
  ...
  if (from === 2) {
    if (!validateQuiz(state.riskAnswers, riskQuestions.length)) {
      alert('Mohon jawab semua pertanyaan profil risiko.');
      return;
    }
  }
  showStep(from + 1);
...
function validateFinancialForm() {
  ...
  if (!income || !expenses || !savings || emergency === '') {
    alert('Mohon lengkapi semua field wajib (penghasilan, pengeluaran, dana dimiliki, dana darurat).');
    return false;
  }
  return true;
}
...
function calculateReadinessScore() {
  ...
  } else if (objectivePct <= 0.2 && fomoAvg <= 2 && fomoValues.length > 0) {
    consistencyFlag = true;
    litScore *= 0.85;
...
function getRecommendations(result) {
  ...
  if (result.phase === 'Belum Siap') {
    ...
    if (f.debt > 0 && f.debtInterest >= 12) {
      recs.push('Lunasi utang berbunga tinggi terlebih dahulu sebelum memikirkan investasi. Bunga utang sering lebih besar dari return investasi.');
    }
    if (s.literacy < 10) {
    ...
    if (s.literacy < 12) {
      recs.push('Lanjutkan belajar: pahami perbedaan instrumen (reksa dana, obligasi, saham) dan cocokkan dengan horizon waktumu.');
    }
    ...
    recs.push('Susun tujuan investasi yang jelas (horizon waktu + target) sebelum memilih instrumen.');
    recs.push('Diversifikasi sesuai profil risiko. Jangan menempatkan semua dana di satu aset hanya karena sedang "hot".');
...
function getReasonText(result) {
  ...
  if (s.risk >= 10) parts.push('profil risiko cukup selaras');
  else parts.push('profil risiko perlu diselaraskan dengan kondisi aktual');
  ...
  return `Skor kamu mencerminkan bahwa ${parts.join('; ')}. Fase "${result.phase}" menunjukkan langkah konkret yang paling relevan untuk kondisi saat ini — bukan penilaian keberhasilan atau kegagalan.`;
}
...
function renderResult(result) {
  ...
  const s = result.scores;
  const fomoLabel = s.fomoAvg <= 2.2 ? 'Rendah' : s.fomoAvg <= 3.5 ? 'Sedang' : 'Tinggi';
  ...
  container.innerHTML = `
    <div class="result-header">
      ...
      <p class="result-summary">Skor Kesiapan Investasi kamu</p>
    </div>
    ...
    <div class="result-breakdown">
      ...
      <div class="breakdown-item">
        ...
        <span class="breakdown-score" style="color:var(--text-soft)">${s.fomoAvg}/5 · ${fomoLabel}</span>
      </div>
      <div class="breakdown-item">
...
function saveToHistory() {
  ...
  alert('Hasil evaluasi berhasil disimpan ke Dashboard.');
  showPage('dashboard');
}

function clearHistory() {
  if (confirm('Hapus semua riwayat evaluasi? Tindakan ini tidak bisa dibatalkan.')) {
...
function renderDashboard() {
  ...
  if (prev) {
    ...
    const sign = diff > 0 ? '+' : '';
    document.getElementById('scoreChange').textContent = sign + diff;
    document.getElementById('scoreChange').style.color = diff >= 0 ? 'var(--primary-light)' : 'var(--danger)';
  } else {
...
function drawScoreChart(history) {
  ...
  if (scores.length === 1) {
    ...
    const y = padding.top + chartH - (scores[0] / 100) * chartH;
